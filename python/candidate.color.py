...
class Candidate(object):
    ...
    def plot(self, figsize=(12, 10), dpi=180):
    ...
    def savefig(self, fname, **kwargs):
        ...
        pms = f"{self.params['period'] * 1000.0:.3f}"
        basename = os.path.basename(basename)
        fname_root, fname_ext = os.path.splitext(fname)
        fname = f"{fname_root}_{pms}ms_{basename}{fname_ext}"
        # fig.savefig(fname)
...
def plot_table(params, tsmeta):
    ...
    # date/time standard
    coord = tsmeta['skycoord']
    ra_hms = coord.ra.to_string(unit=uu.hour, sep=':', precision=4, pad=True)
    dec_hms = coord.dec.to_string(unit=uu.deg, sep=':', precision=4, pad=True)
   ...
   # --------------------------
    ...
    basename = os.path.basename(basename)
    wrapped_text = textwrap.fill(f"File: {basename}", width=75)
    ...
    y=0.1,               # Fixed Y-height: 12% of the subplot's total height (adjust as needed)
    ...
    source_name = tsmeta['source_name']
    wrapped = textwrap.fill(f"Source: {source_name}", width=75) 
    ax.text(
        x=0.02, y=0.19,
        s=wrapped,
...
def plot_dm_curve(dm, snr):
    ...
    dm_max = dm.max()
    #plt.plot(dm, snr, color='r', marker='o', markersize=3)
    plt.plot(dm, snr, marker='o', color='#4F86C6', markeredgecolor='#4F86C6')
    max_snr_idx = snr.argmax()
    max_dm = dm[max_snr_idx]
    max_snr = snr[max_snr_idx]
    plt.axvline(x=max_dm, color='#E71D36', linestyle='--', label='optimal S/N')
    plt.grid(linestyle=':')
...
def plot_subints(X, T):
    ...
    plt.imshow(
        X_norm,
        cmap='viridis',  # Replace viridis with reversed grayscale to mimic Presto's white-black gradient
        interpolation='nearest',
        aspect='auto',
        extent=extent,
    ...
    plt.xlim(0, 1.5)
    plt.ylabel("Time (seconds)")
    plt.xlabel("Pulse Phase (1.5 Periods)")
    plt.tick_params(axis='y', labelrotation=90)
    # Phase axis ticks
...
def plot_subints0(X, T):
    ...
    plt.imshow(
        ...
        interpolation="nearest",
        aspect="auto",
        extent=[-0.5, nbins_ext - 0.5, 0, T],  # Note: t = 0 is at the bottom of the plot
    )
    plt.fill_between([nbins, nbins_ext], [0, 0], [T, T], color="b", alpha=0.08)
    plt.xlim(-0.5, nbins_ext - 0.5)
    plt.ylabel("Time (seconds)")
...
def plot_profile(P):
    ...
    """
    nbins = len(P)
    # Extend profile to 1.5 periods
    P = np.concatenate((P, P[: nbins // 2]))
    nbins_ext = len(P)
    # Convert bin index to phase (0–1.5)
    phase = np.linspace(0, 1.5, nbins_ext, endpoint=False)
    # Plot profile
    plt.plot(phase, P - np.median(P), color='#4F86C6')
    plt.axhline(y=0, color='#E71D36', linestyle='--')
    ymin, ymax = plt.ylim()
...
def plot_candidate(cand):
    ...
    plt.subplot(gs[:5, :7])
    plot_profile(cand.profile)
    plt.subplot(gs[5:17, :7])
    plot_subints(cand.subints, cand.tsmeta['tobs'])
    plt.subplot(gs[:10, 8:])
    plot_table(cand.params, cand.tsmeta)
    plt.subplot(gs[10:17, 8:])
    plot_dm_curve(*cand.dm_curve)
    plt.subplot(gs[18:22, :])
    plot_subints0(cand.subints, cand.tsmeta['tobs'])
   # plt.tight_layout()
