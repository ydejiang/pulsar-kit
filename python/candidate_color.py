...
def plot_dm_curve(dm, snr):
    ...
    plt.plot(dm, snr, marker='o', markersize=12, color='#4F86C6', linewidth=2., markeredgecolor='#4F86C6',markerfacecolor='none', markeredgewidth=2.0)
    max_snr_idx = snr.argmax()
    max_dm = dm[max_snr_idx]
    max_snr = snr[max_snr_idx]
    plt.axvline(x=max_dm, color='red', linestyle='--', linewidth=2.,
                label='optimal S/N')

    # Avoid matplotlib warning when calling xlim() with two equal values 
    #if dm_min == dm_max:
    #    plt.xlim(dm_min - 0.15, dm_min + 0.15)
    #else:
    #    plt.xlim(dm_min - 0.15, dm_max + 0.15)
    plt.grid(linestyle=':')
...
def plot_profile(P):
    ...
    plt.plot(phase, P - np.median(P), color='#4F86C6', lw=2.)
    plt.axhline(y=0, color='#E71D36', linestyle='--', linewidth=1.5)
    ymin, ymax = plt.ylim()
