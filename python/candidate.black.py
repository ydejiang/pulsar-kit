...
def plot_dm_curve(dm, snr):
    ...
    dm_max = dm.max()
    #plt.plot(dm, snr, color='r', marker='o', markersize=3)
    plt.plot(dm, snr, marker='o', color='black', markeredgecolor='black')
    max_snr_idx = snr.argmax()
    max_dm = dm[max_snr_idx]
    max_snr = snr[max_snr_idx]
    plt.axvline(x=max_dm, color='black', linestyle='--', label='optimal S/N')
    plt.grid(linestyle=':')
...
def plot_profile(P):
    ...
    # Plot profile
    plt.plot(phase, P - np.median(P), color='black')
